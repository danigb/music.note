...
vows.describe('music-pitch').addBatch({
  'parse': function () {
    assert.deepEqual(pitch.parse('C#4'), [0, 1, 4, 0])
    assert.deepEqual(pitch.parse('Bb'), [6, -1])
  },
  'pitch str': {
    'str': function () {
      assert.equal(pitch('c'), 'C')
      assert.equal(pitch('bbb3'), 'Bbb3')
      assert.equal(pitch('fx'), 'F##')
      assert.equal(pitch([0, 1, 3, 0]), 'C#3')
    },
    'letter': function () {
    ...
    'pitch class': function () {
      ...
      assert.equal(pitch.pitchClass(null), null)
    },
